  ...
  <ion-page>
    ...
    <ion-list>
      <ion-item
        ...
        :key="order.id"
        button
        @click="() => router.push(`/pending-orders/${order.id}/view`)"
      >
        <ion-label>
          <ion-row>
            <ion-col>
...
export default {
  ...
  async mounted() {
    await this.list(this.filters, true, true);
  },
  ...
  watch: {
    $route(value) {
      if (value && value.path === "/approvel")
        this.list(this.filters, true, true);
    },
  },
};
</script>
